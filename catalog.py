...
def fbconnect():
    ...
    app_id = json.loads(open('/var/www/html/catalog/fb_client_secrets.json',
                             'r').read())['web']['app_id']
    app_secret = json.loads(open(
        '/var/www/html/catalog/fb_client_secrets.json', 'r').read())['web']['app_secret']
    url = 'https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=%s&client_secret=%s&fb_exchange_token=%s' % (
        app_id, app_secret, access_token)
    # url = 'https://graph.facebook.com/oauth/access_token?grant_type=
    # fb_exchange_token&client_id=%s&client_id=%s&client_secret=%s&
...
def showItems(category_name):
    ...
    return render_template('items_per_category.html',
                           categories=categories, itemcategory=category,
                           items=items, numitems=char_numitems)
...
def editItem(item_title):
    ...
    if request.method == 'POST':
        ...
        categories = session.query(Category).all()
        itemsCategory = session.query(Category).filter_by(
            id=editedItem.category_id).one()
        return render_template('edititem.html', item=editedItem,
                               categories=categories, category=itemsCategory)
#
